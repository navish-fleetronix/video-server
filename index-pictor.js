...
function buildQueryRecordings(phone) {
    // CMSV6 always sends a real date range — devices ignore all-zero times
    const now   = new Date();
    const bcd   = n => parseInt(n.toString().padStart(2,'0').split('').map((d,i,a) => i%2===0 ? (parseInt(a[i])<<4)|parseInt(a[i+1]) : null).filter(v=>v!==null).map(v=>String.fromCharCode(v)).join(''), 16);

    // Build BCD bytes manually
    ...
    // Query from 30 days ago to now — covers all recent recordings
    const past = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    const pyy  = past.getFullYear() % 100;
    const pmo  = past.getMonth() + 1;
    const pdd  = past.getDate();

    const body = Buffer.alloc(23);
    body[0] = 0; // all channels
    toBCDBytes(pyy, pmo, pdd, 0, 0, 0).copy(body, 1);   // start: 30 days ago 00:00:00
    toBCDBytes(yy,  mo,  dd, 23,59,59).copy(body, 7);   // end:   today 23:59:59
